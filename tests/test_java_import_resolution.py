...
class JavaImportResolutionTests(unittest.TestCase):
    ...
    def test_import_from_generated_only_package_is_resolved(self):
        from codewiki.index import pipeline
        from codewiki.store.db import open_index

        with tempfile.TemporaryDirectory(prefix="codewiki-import-resolved-") as root:
            ...
            self.assertEqual("resolved", type_outcome)
            ...
            self.assertEqual("resolved", queried.outcome)
    ...
    def test_missing_type_classification_uses_exact_packages_and_wildcard_precedence(self):
        ...
        with tempfile.TemporaryDirectory(prefix="codewiki-type-prefix-") as root:
            ...
            generated_wild = resolve_type_path(
                result.db_path, "Missing", "src/app/ExcludedWild.java"
            )
            self.assertEqual("unresolved", generated_wild.outcome)
            self.assertIsNone(generated_wild.rule)
            self.assertEqual([], generated_wild.candidates)
    ...
    def test_meta_reports_five_resolved_two_unresolved_five_external_zero_excluded(self):
        from codewiki.index import pipeline
        from codewiki.store.db import open_index
        ...
        with tempfile.TemporaryDirectory(prefix="codewiki-import-counts-") as root:
            ...
            self.assertEqual(5.0 / 7.0, result.internal_resolution_rate)
            connection = open_index(result.db_path)
            try:
                meta = dict(connection.execute("SELECT key, value FROM meta"))
            finally:
                connection.close()
            self.assertEqual("0.714286", meta["internal_resolution_rate"])
            self.assertEqual({
                "resolved": 5,
                "unresolved": 2,
                "external": 5,
